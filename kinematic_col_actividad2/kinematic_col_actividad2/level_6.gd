...
func _on_goal_body_entered(body):
	if body.is_in_group("player"):
		...
		$score.set_position(Vector2(770.0, 150.0))
		_set_score_string(0)
		finished = true
...
func _on_big_enemy_2_give_score() -> void:
	$big_enemy_death_sound.play()
	additional_points = additional_points + big_enemy_value
	_set_score_string(big_enemy_value)

func _on_opponent_gone() -> void:
	additional_points = additional_points + opponent_escape_value
	_set_score_string(opponent_escape_value)
...
func _on_breakable_wall_4_destroyed() -> void:
	$force_field_break_sound.play()


func _on_opponent_dead() -> void:
	$opponent_death_sound.play()
